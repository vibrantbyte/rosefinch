...
local redis_factory = function(h)
    ...
    h.connect = function(connect_info)
        ...
        local ok,err = redis_instance:connect(connect_info.host, connect_info.port);
        if  not ok then
            ngx.print("failed to connected. ",err);
            return false,nil;
        end

        if connect_info.pass ~= '' then
            local res,err = redis_instance:auth(connect_info.pass);
            if not res then
    ...
    self.get_instance = function(_, name)
